...
import { CustomFrameworkCard } from "@@/create-project/components/CustomFrameworkCard";
...
import { FrameworkOption } from "@@/create-project/types/types";
...
export function FrameworkSelection() {
  ...
  const { form, state, toggleFramework, goToDatabaseStep } = useProjectForm();
  ...
  const availableFrameworkOptions = useMemo(() => {
    return (
      frameworkOptions[selectedAppType as keyof typeof frameworkOptions] || []
    );
  }, [selectedAppType]);

  // Function to check if a framework is from the predefined list
  const isPredefinedFramework = useCallback(
    (frameworkName: string) => {
      return (
        frameworkOptions[
          selectedAppType as keyof typeof frameworkOptions
        ]?.some((category) =>
          category.options.some((opt) => opt.name === frameworkName),
        ) || false
      );
    },
    [selectedAppType],
      ...
      <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Select framework(s) or technology for your project
          </p>
        </div>
        <Button
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-1 justify-start sm:justify-end mt-2">
                      {framework.primaryLanguages ? (
                        ...
                        framework.primaryLanguage.toLowerCase() !==
                          framework.name.toLowerCase() ? (

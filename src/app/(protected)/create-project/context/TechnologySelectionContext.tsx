...
function technologyReducer(state: TechnologyState, action: TechnologyAction): TechnologyState {
  switch (action.type) {
    case "SET_APP_TYPE":
...
const TechnologySelectionContext = createContext<TechnologySelectionContextType | undefined>(undefined);
...
// Provider component
export function TechnologySelectionProvider({ children }: TechnologySelectionProviderProps) {
  ...
  const setApplicationType = useCallback((type: string) => {
  ...
  const toggleFramework = useCallback((framework: string) => {
    ...
    if (currentFrameworks.includes(framework)) {
      ...
      dispatch({
        type: "SET_FRAMEWORKS",
        payload: [...state.selectedFrameworks, { name: framework, description: '', primaryLanguages: [] }]
      });
    }
  ...
  const isFrameworkSelected = useCallback((framework: string) => {
    const currentFrameworks = form.watch("frameworks") || [];
  ...
  const toggleDatabase = useCallback((database: string) => {
    ...
    if (database === "None") {
      ...
      if (currentDatabases.includes("None")) {
        ...
        dispatch({ 
          type: "SET_DATABASES", 
          payload: [{ name: "None", description: "No database needed for this project" }] 
        });
  ...
  const isDatabaseSelected = useCallback((database: string) => {
    if (database === "None") {
  ...
  const getDatabaseOptions = useCallback(() => {
    ...
    frameworks.forEach(framework => {
      ...
      const dbOpts = (databaseOptions as Record<string, DatabaseOption[]>)[framework] || [];
...
export function useTechnologySelection() {
  ...
  if (!context) {
    throw new Error("useTechnologySelection must be used within a TechnologySelectionProvider");
  }
